...
def test_sim_preparer_end_to_end_with_bundled_forcefield(tmp_path):
    metadata = [("TIP3", 10)]
    ...
    sim_preparer(metadata, str(output_path), pred_path=str(pred_path))
...
def test_sim_preparer_extension_itp_overrides_bundled_one(tmp_path):
    ext_dir = tmp_path / "ext"
    ext_dir.mkdir()
    (ext_dir / "TIP3.itp").write_text("; custom override\n")
    output_path = tmp_path / "out"

    sim_preparer([("TIP3", 1)], str(output_path), ext_path=str(ext_dir))

    copied = (output_path / "toppar" / "TIP3.itp").read_text()
    assert copied == "; custom override\n"


def test_sim_preparer_missing_itp_is_reported_but_does_not_raise(tmp_path, capsys):
    output_path = tmp_path / "out"

    sim_preparer([("NOT_A_REAL_RESIDUE", 1)], str(output_path))
